...
{
   Lora_Send_Cmd_Buf[5] = cmd;   
   Lora_Send_Cmd_Buf[6] = Dev_Num;
	 Lora_Send_Cmd_Buf[7] = cnt;
...
{	
	...
	switch (htons(data->cmd))
	{
		case lora_cmd_finish:
			{
			  APP_Set_Status(Run_Idle);
			}
		break;
		case lora_cmd_rotate_recv:
			{

			  APP_Set_Status(Run_Rotate_RecvRSP);
			}
		break;
		case lora_cmd_recv :
			{
